...
export const StoreInfoFetcher = memo(({
  goToNext,
}: Props): JSX.Element => {
  ...
  const fetchShop = useCallback(async (atwa: AccessTokenWalletAddress) => {
    try {
      const response = await fetch("/api/store/getByAccessToken", {
        ...
        body: JSON.stringify({
          accessToken: atwa.accessToken,
          storeUrl: atwa.storeUrl,
        }),
      });
      if (response.ok) {
    ...
    async (accessTokenWalletAddress: AccessTokenWalletAddress) => {
      try {
        const response = await fetch(
          `/api/accessTokenWalletAddress/${accessTokenWalletAddress.id}`,
          {
            method: "DELETE",
            headers: {
              "Content-Type": "application/json",
...
StoreInfoFetcher.displayName = "StoreInfoFetcher";
